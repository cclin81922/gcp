resource "google_logging_metric" "vpc_new_fwrule_metric" {
  ...
  filter = format("resource.type=\"gce_firewall_rule\" AND logName=\"projects/%s/logs/cloudaudit.googleapis.com%%2Factivity\" AND protoPayload.methodName=(\"beta.compute.firewalls.insert\" OR \"v1.compute.firewalls.insert\") AND operation.last=true severity=\"NOTICE\"", var.gcp_project)
  metric_descriptor {
    metric_kind = "DELTA"
    value_type  = "INT64"
  }
